import { CollectionView, ModelSchema, PropertySchema, Selector, View, validator } from 'bbmn-components';
import _ from 'underscore';
import { CollectionView as CollectionView$1, View as View$1 } from 'backbone.marionette';
import { betterResult, buildViewByKey, camelCase, clone, compareObjects, convertString, getByPath, getOption, isViewClass, mix, setByPath, takeFirst, toBool } from 'bbmn-utils';
import { cssClassModifiersMixin, customsMixin } from 'bbmn-mixins';
import { Collection, isClass } from 'bbmn-core';
...
function fixChildView(control, View$$1) {

	if (View$$1 == null) {
		return;
	}

	var proto = View$$1.prototype;

	var mixWith = [];
	if (!_.isFunction(proto.isSelected)) {
		mixWith.push(selectableViewMixin);
	}
	if (!_.isFunction(proto.template) && !_.isFunction(proto.getText)) {
		mixWith.push({ getText: common$1.getText });
	}
	if (mixWith.length) {
		var _mix;

		View$$1 = (_mix = mix(View$$1)).with.apply(_mix, mixWith);
	}

	return View$$1;
}

var SelectControl = ControlView.extend({
	...
	_initSelector: function _initSelector() {
		var _this = this;
		...
		this.listenTo(this.selector, 'change', function (changes) {
			_.invoke(changes.selected, 'trigger', 'change');
			_.invoke(changes.unselected, 'trigger', 'change');
			_this.setControlValue(_this.selector.getValue());
		});
	},
	_fixChildView: function _fixChildView(childView) {
		var _this2 = this;
		...
		if (!childView) {
			...
			this.childView = function () {
				var original = childView.apply(undefined, arguments);
				return fixChildView(_this2, original) || DefaultChildView;
			};
		}
	},
	_fixChildOptions: function _fixChildOptions(childViewOptions) {
		var childDefs = { selector: this.selector };
		if (_.isFunction(childViewOptions)) {
			childViewOptions = childViewOptions.bind(this);
			this.childViewOptions = function () {
				var original = childViewOptions.apply(undefined, arguments);
		...
		if (type == 'text') {
			...
			if (this.valueOptions.type == 'enum') {
				...
				return _.map(value, function (val) {
					return convertString(val, _this3.valueOptions.type);
				});
			}
		} else {
			return value;
	...
	getSource: function getSource() {
		...
		var type = this.valueOptions.type;
		if (type == 'boolean') {
			var models = _.map(src, function (value, ind) {
	...
	childViewEvents: {
		'toggle:select': function toggleSelect(view, event) {
...
export { Button, buttonMixin, controlMixin, ControlView, controlViewMixin, EditProperty, editPropertyMixin, EditModel, editModelMixin, SchemaErrorView, InputControl as Input, inputMixin, PromiseBar, promiseBarMixin, controls, defineControl, getControl, SelectControl, selectableViewMixin };
