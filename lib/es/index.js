...
import { betterResult, buildViewByKey, camelCase, clone, compareObjects, convertString, getByPath, getOption, isEmptyValue, isViewClass, mix, setByPath, takeFirst, toBool } from 'bbmn-utils';
	...
	return Base.extend({
		...
		constructor: function constructor(options) {
			...
			if (this.getOption('destroyOnDone')) {
				this.once('control:done', function () {
					_this.destroy();
				});
			}
...
var controlViewMixin = (function (Base) {
	...
	return Mixed.extend({
		...
		settleButtonsListeners: function settleButtonsListeners(buttonsView) {
			...
			this.on('control:done', function () {
				_this3.triggerMethod('resolve', _this3.getControlValue());
			});
...
var SelectControl = ControlView.extend({
	...
	_initSelector: function _initSelector() {
		...
		this.listenTo(this.selector, 'change', function (changes) {
			...
			var setPromise = _this.setControlValue(_this.selector.getValue());

			if (!_this.isMultiple() && _this.getOption('doneOnSelect')) {
