import { CollectionView, ModelSchema, PropertySchema, View, validator } from 'bbmn-components';
import _ from 'underscore';
import { CollectionView as CollectionView$1, View as View$1 } from 'backbone.marionette';
import { betterResult, buildViewByKey, camelCase, compareObjects, convertString, flat, getByPath, getOption, isViewClass, mix, setByPath, takeValue, unflat } from 'bbmn-utils';
import { cssClassModifiers, customsMixin } from 'bbmn-mixins';
import { isClass, isCollectionView } from 'bbmn-core';
...
var controlViewMixin = (function (Base) {
	...
	return Mixed.extend({
		...
		_buildNestedTextView: function _buildNestedTextView(key, options) {
			...
			return buildViewByKey.call(this, key, { TextView: TextView, buildText: buildText, options: options });
		},
		...
		buildButtonsView: function buildButtonsView() {
			...
			var view = buildViewByKey.call(this, 'buttonsView', { options: options });
			if (!view) {
				return;
			}
...
var editPropertyMixin = (function (Base) {
	var Mixed = mix(Base).with(ControlView, common);
	...
	return Mixed.extend({
		...
		_getHeaderView: function _getHeaderView() {
			...
			var view = buildViewByKey.call(this, 'header', { TextView: TextView, buildText: buildText, options: { tagName: 'header' } });
			if (view) {
				return view;
			}
...
var Input = inputMixin(View);

var TextArea = inputMixin(View).extend({
	doneOnEnter: false,
	tagName: 'textarea',
...
var Textarea = ControlView.extend({
	...
	_delegateInputEvents: function _delegateInputEvents() {
		...
		_.each(inputEvents, function (name) {
			var handlerName = camelCase('on', name);
...
export { Button, buttonMixin, controlMixin, ControlView, controlViewMixin, EditProperty, editPropertyMixin, EditModel, editModelMixin, Input, inputMixin, PromiseBar, promiseBarMixin };
