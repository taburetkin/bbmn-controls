				...
				'click': function click(e) {
					...
					if (before && before.then) {
						before.then(function (data) {
							return _this.triggerClick(data, event);
						}, function (error) {
							return _this.triggerError(error, event);
						});
					} else {
...
var controlMixin = (function (Base) {
	return Base.extend({
		...
		_getEventContext: function _getEventContext(controlName) {
			...
			return { control: control, controlName: controlName, isControlWraper: isControlWraper };
		},
...
function fixAttributes(attrs, view, opts) {
	var tagName = getOption(view, opts, 'tagName');
...
export { Button, buttonMixin, controlMixin, ControlView, controlViewMixin, EditProperty, editPropertyMixin, EditModel, editModelMixin, SchemaErrorView, InputControl as Input, inputMixin, TextAreaControl, PromiseBar, promiseBarMixin, controls, defineControl, getControl, SelectControl, selectableViewMixin, BooleanSwitchControl };
