...
import { betterResult, buildViewByKey, camelCase, compareObjects, convertString, flat, getByPath, getOption, mix, setByPath, takeValue, unflat } from 'bbmn-utils';
import { cssClassModifiers, customsMixin } from 'bbmn-mixins';
import { isClass, isCollectionView } from 'bbmn-core';
import { ModelSchema, PropertySchema, validator } from 'bbmn-components';
		...
		_buildNestedTextView: function _buildNestedTextView(key, options) {
			...
			if (!TextView) {
				buildText = function buildText(text, opts) {
					return new View(_.extend({}, opts, { template: function template() {
							return text;
						} }));
				};
			}
		...
		_getHeaderView: function _getHeaderView() {
			...
			if (this.getOption('propertyLabelAsHeader')) {
				...
				if (TextView) {
					...
					return new View({ template: function template() {
							return label;
						}, tagName: 'header' });
				}
			}
...
var promiseBar = (function (Base) {
	...
	return Mixed.extend({
		...
		buildButton: function buildButton(name) {
			...
			if (!Button) {
				Button = this.buttonView = button(View);
			}
...
export { button, control, controlView, EditProperty, EditModel, input$1 as input, promiseBar };
