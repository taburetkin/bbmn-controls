(function (global, factory) {
	typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('underscore'), require('backbone.marionette'), require('bbmn-utils'), require('bbmn-mixins'), require('bbmn-core'), require('bbmn-components')) :
	typeof define === 'function' && define.amd ? define(['exports', 'underscore', 'backbone.marionette', 'bbmn-utils', 'bbmn-mixins', 'bbmn-core', 'bbmn-components'], factory) :
	(factory((global.bbmn = global.bbmn || {}, global.bbmn.utils = {}),global._,global.Mn,global.bbmnUtils,global.bbmnMixins,global.bbmnCore,global.bbmnComponents));
}(this, (function (exports,_,backbone_marionette,bbmnUtils,bbmnMixins,bbmnCore,bbmnComponents) { 'use strict';

_ = _ && _.hasOwnProperty('default') ? _['default'] : _;

var button = (function (Base) {
	if (Base == null) {
...
var control = (function (Base) {
	return Base.extend({
		...
		_initControl: function _initControl() {
			...
			this._cntrl = {};
			var name = bbmnUtils.takeValue('controlName', options, this) || 'control';
			this._cntrl.name = name;

			var value = bbmnUtils.takeValue('value', options, this);
		...
		initParentControl: function initParentControl(options) {
			var parent = bbmnUtils.takeValue('proxyTo', options, this) || bbmnUtils.takeValue('parentControl', options, this);
  ...
  */
		_clone: function _clone(value) {
			if (_.isArray(value)) return value.slice(0);else if (_.isObject(value)) {
				return bbmnUtils.unflat(bbmnUtils.flat(value));
			} else return value;
		},
...
var controlView = (function (Base) {
	...
	if (!Mixed.CssClassModifiersMixin) {
		Mixed = bbmnMixins.cssClassModifiers(Mixed);
	}
	...
	return Mixed.extend({
		...
		buildErrorView: function buildErrorView() {
			return bbmnUtils.buildViewByKey.call(this, 'errorView');
		},
		...
		_buildNestedTextView: function _buildNestedTextView(key, options) {
			...
			return bbmnUtils.buildViewByKey.call(this, key, { TextView: TextView, buildText: buildText, options: options });
		},
		...
		buildButtonsView: function buildButtonsView() {
			...
			var view = bbmnUtils.buildViewByKey.call(this, 'buttonsView', { options: options });
			if (!view) {
				return;
			}
		...
		getControlView: function getControlView() {
			this.control = bbmnUtils.buildViewByKey.call(this, 'controlView', { options: { parentControl: this, value: this.getControlValue() } });
			return this.control;
		},
...
var EditProperty = (function (Base) {
	var Mixed = bbmnUtils.mix(Base).with(controlView, common);
	...
	return Mixed.extend({
		...
		_getHeaderView: function _getHeaderView() {
			...
			var view = bbmnUtils.buildViewByKey.call(this, 'header', { TextView: TextView, buildText: buildText, options: { tagName: 'header' } });
			if (view) {
				return view;
			}
...
var EditModel = (function (Base) {
	...
	return Mixed.extend({
		...
		_createEditProperty: function _createEditProperty(name, modelSchema) {
			...
			var EditProperty$$1 = this.getEditPropertyClass();
			var def = {
				controlName: name,
				schema: schema,
		...
		createEditProperty: function createEditProperty(EditProperty$$1, options) {
...
var input$1 = (function (Base) {

	var Mixin = Base.ControlMixin ? Base : control(Base);
...
var promiseBar = (function (Base) {
	if (Base == null) {
		Base = backbone_marionette.CollectionView;
	}

	if (!bbmnCore.isCollectionView(Base)) {
	...
	return Mixed.extend({
		...
		addButtons: function addButtons() {
			...
			while (buttons.length) {
				var button$$1 = buttons.pop();
				var preventRender = !!buttons.length;
				this.addChildView(button$$1, { preventRender: preventRender });
			}
		},
		buildButtons: function buildButtons() {
			...
			return _.reduce(names, function (buttons, name) {
				var button$$1 = _this.buildButton(name);
				button$$1 && buttons.push(button$$1);
				return buttons;
			}, []);
		},
		buildButton: function buildButton(name) {
			...
			if (!Button) {
				Button = this.buttonView = button(backbone_marionette.View);
			}
...
var index = {
	...
	EditProperty: EditProperty, EditModel: EditModel,
	input: input$1,
	promiseBar: promiseBar
};
