this.bbmn = this.bbmn || {};
this.bbmn.utils = (function (exports,bbmnComponents,_,backbone_marionette,bbmnUtils,bbmnMixins,bbmnCore) {
'use strict';
...
// const ExtView = mix(View).with(cssClassModifiers);
...
var Input = inputMixin(bbmnComponents.View);
...
var Textarea = ControlView.extend({
	constructor: function constructor() {
...
var controls = {
	'textarea': function textarea() {
		...
		return new Textarea({ value: value });
	},
	'enum:buttons': function enumButtons() {}
};

function getControlByString(name) {
	var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

	var build = controls[name];
	if (!build) return;
	return build(options);
}

function guesControl() {
	var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	var _options$valueOptions = options.valueOptions,
	    valueOptions = _options$valueOptions === undefined ? {} : _options$valueOptions;
	var inputType = valueOptions.inputType;
...
function buildControl() {
	var options = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
	var _options$valueOptions2 = options.valueOptions,
	    valueOptions = _options$valueOptions2 === undefined ? {} : _options$valueOptions2,
	    _options$editOptions = options.editOptions,
	    editOptions = _options$editOptions === undefined ? {} : _options$editOptions,
	    name = options.name,
	    label = options.label,
	    value = options.value;
	var control = editOptions.control;


	if (control) {
		if (bbmnUtils.isViewClass(control)) {
...
var EditProperty = BaseEditProperty.extend({
	buildPropertyView: function buildPropertyView() {
		var editOptions = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};


		var control = buildControl(editOptions);
		if (control) {
			return control;
		}

		var options = _.extend({}, editOptions, {
			inputAttributes: {
...
var index = {
	...
	Input: Input, inputMixin: inputMixin,
...
}({},bbmnComponents,_,Mn,bbmnUtils,bbmnMixins,bbmnCore));
