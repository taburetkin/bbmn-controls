...
function fixChildView(control, View$$1) {
	...
	if (mixWith.length) {
		...
		View$$1 = (_mix = bbmnUtils.mix(View$$1)).with.apply(_mix, mixWith);
	}
...
var SelectControl = ControlView.extend({
	...
	_fixChildView: function _fixChildView(childView) {
		...
		} else if (bbmnUtils.isViewClass(childView)) {
		...
		if (type == 'text') {
			...
			if (this.valueOptions.type == 'enum') {
				...
				return _.map(value, function (val) {
					return bbmnUtils.convertString(val, _this3.valueOptions.type);
				});
			}
		} else {
			return value;
